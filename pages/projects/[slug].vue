<template>
  <div class="min-h-screen bg-slate-950 text-slate-300 selection:bg-emerald-500/30 selection:text-emerald-200">
    ...
    <div class="fixed top-0 left-0 w-full h-[2px] bg-white/5 z-50">
      <div 
    ...
    <div class="mx-auto w-full max-w-5xl px-6 py-6 lg:px-12">
      ...
      <NuxtLink 
        to="/" 
        class="group inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-emerald-400 transition-all duration-300 mb-4"
      >
      ...
      <div v-if="pending" class="space-y-8 animate-pulse">
        <div class="h-10 bg-slate-800 rounded-lg w-3/4"></div>
        <div class="h-6 bg-slate-800 rounded-lg w-1/2"></div>
        <div class="aspect-video bg-slate-800 rounded-2xl w-full"></div>
      </div>

      <!-- Error State -->
      <div v-else-if="error || !project" class="text-center py-20">
        <Icon name="ri:error-warning-line" class="w-16 h-16 text-slate-800 mx-auto mb-4" />
        <h1 class="text-2xl font-bold text-white">Project not found</h1>
        <p class="mt-2 text-slate-400">The project you are looking for does not exist or has been moved.</p>
        <NuxtLink to="/" class="mt-8 inline-block px-6 py-2 bg-emerald-500 text-slate-950 font-bold rounded-full">Return Home</NuxtLink>
      </div>
      ...
      <div v-else class="space-y-12">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center">
          ...
          <div class="lg:col-span-7 space-y-4">
            <h1 class="text-3xl md:text-4xl lg:text-5xl font-black text-white tracking-tight leading-[1.1]">
              {{ project.title }}
            </h1>
            ...
            <div class="flex flex-wrap gap-1.5">
              <span 
                ...
                :key="tech"
                class="px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-bold text-emerald-400 uppercase tracking-tight"
              >
                {{ tech }}
              </span>
            </div>

            <p class="text-sm md:text-base text-slate-400 leading-relaxed font-medium max-w-2xl">
              {{ project.description }}
            </p>
          ...
          <div v-if="project.image" class="lg:col-span-5 relative group order-first lg:order-last">
            <div class="absolute -inset-2 bg-linear-to-r from-emerald-500 to-blue-500 rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-1000"></div>
        ...
        <!-- Markdown Content -->
        <div class="prose prose-invert prose-emerald prose-sm max-w-none prose-headings:font-black prose-headings:tracking-tight prose-headings:mb-3 prose-p:mb-4 prose-a:text-emerald-400 prose-img:rounded-xl">
          <ContentRenderer :value="project" />
        </div>
        
        <!-- Share Section -->
        <div class="mt-6 pt-6 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            <span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Share this work</span>
            <SocialShareButtons
              :title="project.title"
        ...
        <section class="pt-8">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-[9px] font-bold uppercase tracking-[0.2em] text-emerald-400">Other Projects</h2>
            <NuxtLink to="/#projects" class="text-[9px] font-bold text-slate-500 hover:text-emerald-400 transition-colors uppercase tracking-widest flex items-center gap-1.5">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <NuxtLink
              ...
              class="block group"
            >
              <article class="glass glass-hover rounded-2xl p-4 transition-all duration-500 hover:-translate-y-2 h-full flex flex-col">
                ...
                <p class="mt-2 text-sm text-slate-400 line-clamp-2 leading-relaxed grow">
    ...
    <footer class="mt-12 pt-8 pb-12 border-t border-white/5">
